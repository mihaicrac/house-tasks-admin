...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    private void sendRegistrationToServer(String token) {
        if (notificationsClient == null || tokenStoreService == null) {
            ((MyApplication) getApplicationContext()).getAppComponent().inject(this);
        }
        String userId = tokenStoreService.getUserId();
        if (userId != null && !userId.isEmpty()) {
            RegisterTokenDto registerTokenDto = new RegisterTokenDto();
            registerTokenDto.setUserId(UUID.fromString(userId));
            registerTokenDto.setToken(token);
            notificationsClient.registerToken(registerTokenDto);
        } else {
            tokenStoreService.saveFcmToken(token);
        }
    }
